...
const templateEngine = {
  ...
  extractOptimizedBodyContent(result) {
    const actualResult = result.result?.result || result.result || result;
    const blocks = actualResult?.blocks || [];
    // Get FIRST optimized paragraph for CTA preview (hero section)
    const firstParagraph = blocks.find(b => b.selector && b.selector.includes('p'));
    
    if (!firstParagraph) return null;
    
    const bodyText = firstParagraph.optimized_text || firstParagraph.new_text || firstParagraph.text;
    
    // Return full text without truncation for CTA preview
