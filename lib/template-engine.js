...
const templateEngine = {
  ...
  extractContentBlocksForComparison(result) {
    ...
    const contentBlocks = blocks
      ...
      .map((block, index) => {
        // Use ID-based matching for perfect content association
        let originalBlock = null;
        
        // First try: exact ID match (most reliable)
        if (block.id) {
          originalBlock = originalBlocks.find(b => b.id === block.id);
        }
        
        // Second try: exact selector match
        if (!originalBlock) {
          originalBlock = originalBlocks.find(b => b.selector === block.selector);
        }
        
        // Third try: index-based match as fallback
        if (!originalBlock) {
        ...
        const blockData = {
          ...
          original_text: originalBlock?.text || `Original content for block ${block.id || (index + 1)}`,
