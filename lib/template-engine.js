    ...
    const contentBlocks = blocks
      ...
      .map((block, index) => {
        ...
        if (!originalBlock && block.selector === 'p') {
          ...
          // Find the first few meaningful paragraph blocks (skip very short ones)
          const meaningfulParagraphs = paragraphBlocks.filter(b => b.text && b.text.trim().length > 20);
          
          // For now, use simple index-based matching but ensure we get the right content
          // The AI seems to be creating new content rather than optimizing existing paragraphs
          originalBlock = meaningfulParagraphs[index] || null;
          
          // If we still don't have a good match, try to find content with similar length
          if (!originalBlock || originalBlock.text.length < 50) {
            const similarLengthBlocks = meaningfulParagraphs.filter(b => 
              Math.abs(b.text.length - (block.optimized_text?.length || 0)) < 100
            );
            originalBlock = similarLengthBlocks[0] || originalBlock;
          }
        }
