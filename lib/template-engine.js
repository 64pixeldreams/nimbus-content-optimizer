...
const templateEngine = {
  ...
  extractOptimizedBodyContent(result) {
    const actualResult = result.result?.result || result.result || result;
    const blocks = actualResult?.blocks || [];
    // Get first 2-3 optimized paragraphs for above-fold body content
    const bodyParagraphs = blocks.filter(b => b.selector && b.selector.includes('p')).slice(0, 2);
    const bodyText = bodyParagraphs.map(p => p.optimized_text || p.new_text || p.text).join(' ');
    return bodyText ? bodyText.substring(0, 200) + (bodyText.length > 200 ? '...' : '') : null;
  },
