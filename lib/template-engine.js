...
const templateEngine = {
  ...
  formatIndividualPreviewData(result, originalData, profile, optimizationType, tierLevel) {
    return {
      ...
      content_blocks: (() => {
        const blocks = this.extractContentBlocksForComparison(result);
        console.log('DEBUG: Content blocks extracted:', blocks.length, 'blocks');
        return blocks;
      })()
    };
  },
  ...
  extractPrimaryCTA(result) {
    ...
    const ctas = result.links || result.result?.links || result.result?.result?.links || [];
    if (ctas && ctas.length > 0) {
      return ctas[0].new_anchor || ctas[0].anchor;
    }
    return null;
  },
  ...
  extractSecondaryCTA(result) {
    ...
    if (ctas && ctas.length > 1) {
      return ctas[1].new_anchor || ctas[1].anchor;
    }
    return null;
  },
  ...
  extractOptimizedPrimaryCTA(result) {
    // Handle both nested and direct structures
    const ctas = result.links || result.result?.links || result.result?.result?.links || [];
    const primaryCta = ctas.find(c => c.action === 'upgrade' || ctas.indexOf(c) === 0);
  ...
  extractOptimizedBodyContent(result) {
    // Handle both nested and direct block structures
    const blocks = result.result?.blocks || result.blocks || result.result?.result?.blocks || [];
  ...
  extractContentBlocksForComparison(result) {
    ...
    console.log('DEBUG: Result structure:', {
      ...
      actualBlocks: blocks.slice(0, 2) // Show first 2 blocks
    });
    
    // Get all AI-optimized content blocks
    const contentBlocks = blocks
      .filter(b => b.optimized_text && b.optimized_text.trim()) // Only blocks with optimized text
      .map((block, index) => {
        const blockData = {
          id: block.id || (index + 1).toString(),
          selector: `Block ${block.id || (index + 1)}`,
          original_text: block.original_text || `[Original content for ${block.id || (index + 1)}]`,
          optimized_text: block.optimized_text,
          type: 'content',
          optimization_notes: block.optimization_notes || ''
        };
        
        return blockData;
      });
    
    console.log('DEBUG: Content blocks extracted:', contentBlocks.length, 'blocks');
  ...
  extractOptimizedSecondaryCTA(result) {
    // Handle both nested and direct structures
    const ctas = result.links || result.result?.links || result.result?.result?.links || [];
    const secondaryCta = ctas.find(c => c.action === 'upgrade' && ctas.indexOf(c) === 1);
