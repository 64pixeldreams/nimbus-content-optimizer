...
const templateEngine = {
  ...
  formatIndividualPreviewData(result, originalData, profile, optimizationType, tierLevel) {
    return {
      ...
      optimized_primary_cta_url: this.extractOptimizedPrimaryCTAUrl(result),
      optimized_secondary_cta: this.extractOptimizedSecondaryCTA(result),
      optimized_secondary_cta_url: this.extractOptimizedSecondaryCTAUrl(result),
  ...
  extractOptimizedPrimaryCTAUrl(result) {
    const actualResult = result.result?.result || result.result || result;
    const ctas = actualResult?.ctas || actualResult?.links || [];
    const primaryCta = ctas.find(c => c.cta_type === 'primary' || c.conversion_priority === 'high');
    return primaryCta?.new_href || primaryCta?.href || '/start-repair.html';
  },
  ...
  extractContentBlocksForComparison(result) {
    ...
    const contentBlocks = blocks
      ...
      .map((block, index) => {
        // Smart content matching without relying on AI-provided IDs
        let originalBlock = null;
        
        // Strategy 1: Find original block by content similarity
        if (block.optimized_text || block.text) {
          const aiText = (block.optimized_text || block.text).toLowerCase();
          
          // Look for original blocks with similar content themes
          originalBlock = originalBlocks.find(b => {
            if (!b.text) return false;
            const originalText = b.text.toLowerCase();
            
            // Check for keyword matches (pricing, battery, repair, etc.)
            const keywords = ['pricing', 'price', 'battery', 'repair', 'service', 'guarantee', 'shipping', 'vat'];
            const hasKeywordMatch = keywords.some(keyword => 
              originalText.includes(keyword) && aiText.includes(keyword)
            );
            
            // Check for length similarity (AI usually maintains similar content length)
            const lengthSimilarity = Math.abs(originalText.length - aiText.length) < 100;
            
            return hasKeywordMatch && lengthSimilarity;
          });
        }
        
        // Strategy 2: Position-based matching (fallback)
        if (!originalBlock) {
          const paragraphBlocks = originalBlocks.filter(b => 
            b.type === 'p' && b.text && b.text.length > 20
          );
          
          if (paragraphBlocks[index]) {
            originalBlock = paragraphBlocks[index];
          } else if (paragraphBlocks.length > 0) {
            // Use the most relevant paragraph based on position
            const fallbackIndex = Math.min(index, paragraphBlocks.length - 1);
            originalBlock = paragraphBlocks[fallbackIndex];
          }
        }
        
        // Strategy 3: Use the first meaningful paragraph if all else fails
        if (!originalBlock) {
          const meaningfulBlocks = originalBlocks.filter(b => 
            b.type === 'p' && b.text && b.text.length > 20
          );
          if (meaningfulBlocks.length > 0) {
            originalBlock = meaningfulBlocks[0];
          }
        }
        ...
        const blockData = {
          ...
          original_text: originalBlock?.text || `Original content for block ${index + 1}`,
  ...
  extractOptimizedSecondaryCTAUrl(result) {
    const actualResult = result.result?.result || result.result || result;
    const ctas = actualResult?.ctas || actualResult?.links || [];
    const secondaryCta = ctas.find(c => c.cta_type === 'secondary' || (c.conversion_priority !== 'high' && ctas.indexOf(c) === 1));
    return secondaryCta?.new_href || secondaryCta?.href || 'https://uk.trustpilot.com/review/repairsbypost.com';
  },
